/* Co-firing manual manure usage V1. Separate from morning-meeting/Daily DATA.
...
export async function onRequestGet(context) {
  try {
    ...
    console.error('cofiring manure read failed:',error instanceof Error?error.message:'unknown error');
    return json({ok:false,message:'축분 저장값을 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.'},500);
  }
}
export async function onRequestPost(context) {
  try {
    ...
    if (req.headers.get('X-ShiftLog-Client')!=='desktop' || /Android|iPhone|iPad|iPod|Mobile/i.test(req.headers.get('User-Agent')||''))
      return json({ok:false,message:'축분 입력·수정은 로그인한 PC 화면에서 가능합니다.'},403);
    ...
    if ((body.action==='save' && tons===null) || (body.action==='clear' && body.tons!==null))
      return json({ok:false,message:'축분 사용량은 0 이상, 소수점 6자리 이하로 입력해 주세요. 빈칸은 0으로 저장되지 않습니다.'},400);
    const db = context.env.DB;
    ...
    console.error('cofiring manure save failed:',error instanceof Error?error.message:'unknown error');
