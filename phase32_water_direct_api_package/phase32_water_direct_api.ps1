...
Write-Host '===== Phase 3.2 Water Direct API ====='

Write-Host ''
Write-Host '===== Locate pre-Phase3.1 backup ====='

$Stable =
  Get-ChildItem `
    (Join-Path $Repo 'local-tools\ois-agent') `
    -Filter 'ois-login.js.before-phase31-water-probe-*.bak' `
    -File `
...
if (-not $Stable) {
  throw 'Pre-Phase3.1 backup was not found.'
}
...
if (
  ...
  $StableText.Contains('[PHASE3.1 WATER DIRECT PROBE]')
) {
  throw 'Selected stable backup unexpectedly contains Phase 3 trace/probe code.'
}
...
$CurrentBackup =
  "$Agent.before-phase32-water-direct-$Stamp.bak"
...
Write-Host ''
Write-Host '===== Restore clean pre-probe Agent ====='
...
/* =========================================================
  ...
  Direct same-origin OIS API path for Water treatment.

  Verified against the existing UI result on multiple dates:
  ...
  Existing UI flow remains as fallback if direct data is invalid.
========================================================= */
