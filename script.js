...
async function getPreviousShiftHandoverEntries(
  ...
  /*
    저장 시점에 따라 구분명의
    띄어쓰기가 다르더라도 같은 구분으로 처리한다.
  */
  const normalizeHandoverCategory = (
    value
  ) => {
    const categoryText =
      String(
  ...
  /*
    같은 날짜·근무·보직의 일지가 여러 개면
    가장 최근에 수정된 일지를 사용한다.
  */
  const findLatestPreviousLog = (
    sourceLogs
  ) => {
    const safeLogs =
      Array.isArray(
  ...
  /*
    ...
    handoverEntries가 있으면 먼저 사용하고,
    없으면 기존 entries 배열을 사용한다.
  */
  const collectHandoverEntries = (
    sourceLog
  ) => {
    if (
      !sourceLog ||
    ...
    const appendCandidate = (
      ...
      /*
        TM 발행은 위쪽 TM 발행 내역 영역이며,
        비고는 아래쪽 비고 영역이므로 제외한다.
      */
      if (
        category ===
          "TM 발행" ||
      ...
      */
      ...
      candidates.push({
        ...
        importedFromAuthor:
          String(
            sourceEntry.importedFromAuthor ||
            sourceLog.author ||
            ""
          ).trim(),
    ...
    */
    ...
    /*
      handoverEntries와 entries에
      같은 항목이 중복 저장된 경우 한 번만 사용한다.
        ...
        uniqueEntryMap.set(
          uniqueKey,
          {
            ...
            */
            ...
            importedFromRole:
              candidate.importedFromRole ||
              normalizedRole,

            importedFromAuthor:
              candidate.importedFromAuthor,

            importedFromLogId:
              candidate.importedFromLogId,
  ...
  /*
    1. 현재 불러온 신규 업무일지에서 조회
  */
  const currentPreviousLog =
    findLatestPreviousLog(
      appState.logs
    );
...
async function loadPreviousShiftHandoverEntriesForCurrentEditor() {
  const editorForm =
  ...
  */
  ...
  /*
    한 항목 안의 줄바꿈은 유지하고
    맨 앞에 저장된 기존 번호만 제거한다.
  */
  const normalizeHandoverContent = (
    value
  ) => {
    return String(
      value ||
  ...
  /*
    구분·시간·TAG·내용이 모두 같을 때만
    동일한 항목으로 판단한다.
  */
  const createHandoverEntryKey = (
    entry
  ) => {
    const category =
      normalizeHandoverCategory(
  ...
  appState.previousShiftHandoverRequestToken =
    requestToken;
  ...
  try {
    const previousHandoverResult =
      await getPreviousShiftHandoverEntries(
        currentRole,
        currentDate,
        currentShift
      );
    ...
    if (
      appState
        .previousShiftHandoverRequestToken !==
      requestToken
    ) {
      return;
    }
    ...
    */
    ...
    const sourceEntries =
      Array.isArray(
        previousHandoverResult?.entries
      )
        ? previousHandoverResult.entries
        : [];
    ...
    if (
      ...
      showToast(
        `${currentRole} 전 근무 업무일지에 인계사항이 없습니다.`
      );
    ...
    */
      ...
      (
        ...
        importedEntries.push({
          ...
          */
          ...
          inheritedFromDate:
            String(
              previousHandoverResult?.date ||
              ""
            ).trim(),

          inheritedFromShift:
            String(
              previousHandoverResult?.shift ||
              ""
            )
              .trim()
              .toUpperCase()
    ...
    showToast(
      `${previousHandoverResult.date} ${previousHandoverResult.shift} ${currentRole} 인계사항 ${importedEntries.length}건을 가져왔습니다.`
    );

  } catch (error) {
    console.error(
      `${currentRole} 전 근무 인계사항 가져오기 실패:`,
      error
    );


    showToast(
      "전 근무 인계사항을 가져오지 못했습니다."
    );
...
function getMemberLogEntriesForImport(
  ...
  */
  ...
  /*
    분리 배열이나 entries에 비고가 없으면
    기존 note 문자열을 비고 항목으로 추가한다.
  */
...
/* =========================================================
  ...
  설비 구성과 문구는 고정하며,
  사용자는 수정창에서 운전 상태만 변경한다.
========================================================= */

function createFixedOperationStatusItemsByRole(
  role
) {
  const normalizedRole =
    normalizeMemberLogRole(
...
function createDefaultOperationStatus(
  ...
  /* =====================================================
    TGO·BCO1·BCO2 고정 설비 운전현황
  ====================================================== */

  if (
    equipmentRoles.includes(
      normalizedRole
    )
  ) {
    const operationItems =
      createFixedOperationStatusItemsByRole(
        normalizedRole
      );
  ...
  /* =====================================================
    ...
    실제 화면에서는 createLeaderCombinedOperationStatus()
    결과를 사용한다.
  ====================================================== */
...
/* =========================================================
  ...
  TGO·BCO1·BCO2:
  - 날짜와 근무에 관계없이 보직별로 하나만 저장
  - 마지막으로 수정한 운전상태를 계속 유지

  TO·BO1·BO2:
  - 날짜 + 근무 + 보직별로 저장
  - 직전 근무 운전현황 가져오기 방식 유지

  파트장:
  - 별도 운전현황을 저장하지 않음
  - TGO·BCO1·BCO2 최신 상태 자동 취합
========================================================= */
...
function getOperationStatusStorageKey(
  ...
  /*
    TGO·BCO1·BCO2는 날짜·근무와 관계없이
    보직별 고정 운전현황 하나만 사용한다.
  */
  if (
    fixedEquipmentRoles.includes(
      normalizedRole
    )
  ) {
    return [
...
/* =========================================================
  ...
  TGO·BCO1·BCO2:
  1. 보직별 고정 저장값 조회
  2. 없으면 현재 날짜·근무의 기존 저장값 조회
  3. 기존 값이 있으면 고정 저장값으로 자동 이전
  4. 모두 없으면 고정 설비 기본값 생성
...
function loadOperationStatusByRole(
  ...
  /* =====================================================
    2. TGO·BCO1·BCO2 기존 날짜·근무별 저장값 이전

    이전 키:
    gsShiftLog.operationStatus||날짜||근무||보직

    신규 키:
    gsShiftLog.operationStatus||fixed||보직
  ====================================================== */

  if (
    isFixedEquipmentRole
  ) {
...
function createLeaderCombinedOperationStatus() {
  ...
  /* =====================================================
    ...
    설비 순서대로 하나의 문자열을 만든다.
    [TGO], [BCO1] 같은 중간 제목은 넣지 않는다.
  ====================================================== */
...
/* =====================================================
  전 근무 운전현황 가져오기 버튼 표시 규칙

  TGO·BCO1·BCO2:
  - 운전현황 고정
  - 전 근무 운전현황 가져오기 버튼 숨김
  - 수정 버튼으로 운전상태만 변경
...
/* =========================================================
  ...
  TGO·BCO1·BCO2
  - 해당 날짜·근무에 저장된 고정 운전현황 표시
  - 전 근무 운전현황 자동 가져오기 안 함
  - 필요할 때 수정 버튼으로 변경
...
async function openLogEditor(
  ...
  */
  ...
  if (
    ...
    */
    if (
      ...
      const matchedShiftOption = [
        ...elements.logShift.options
      ].find(
        option => {
          return (
            String(
  ...
  /* =====================================================
    ...
    refreshOperationStatusForCurrentRole() 내부 규칙:

    TGO·BCO1·BCO2
    → 현재 날짜·근무의 고정 운전현황

    TO·BO1·BO2
    → 현재 저장값이 없으면 직전 근무 운전현황

    파트장
    → TGO·BCO1·BCO2 자동 취합
  ====================================================== */

  await refreshOperationStatusForCurrentRole();


  /*
    파트장 팀원 업무일지 취합 영역 갱신
  */
  updateMemberLogImportSection();
...
function collectLogEntriesForDisplay(log) {
  ...
  */
  ...
  const getEntryContentKey =
    content => {
      return normalizeMemberImportContent(
        String(
          content || ""
        )
          .normalize(
...
/* =========================================================
  파트장 상세보기 항목의 실제 출처 보직 판정 최종본

  판정 순서:
  1. 정확히 저장된 팀원 보직
  2. 원본 업무일지 ID
  3. 원본 작성자
  4. 같은 날짜·근무의 팀원 업무내용 비교
  5. 일치하지 않으면 파트장 직접 업무

  중요:
  importedFromRole이 "파트장"이어도 즉시 확정하지 않는다.
...
function createSearchLogPreviewHtml(log) {
  /*
    조회 미리보기를 별도로 만들지 않고
    메인 업무일지 목록의 미리보기 HTML을 그대로 사용한다.
  ...
  */
  ...
  /*
    createLogRowHtml() 결과가 tr이므로
    table > tbody 안에서 안전하게 분석한다.
  */
  template.innerHTML = `
    <table>
      <tbody>
  ...
  */
  ...
  /*
    메인 목록의 button 자체는 복사하지 않고
    내부 미리보기 구조만 사용한다.

    행·업무내용 클릭은 기존
    renderSearchResults()가 처리한다.
  */
  return `
    <span
      class="
...
/* =========================================================
  ...
  적용 방식:
  - 같은 보직의 직전 근무 운전현황 조회
  - 줄바꿈과 내용 그대로 적용
  - 현재 작성 내용이 있으면 덮어쓰기 확인
  - 운전현황 저장 버튼을 눌러야 현재 근무에 확정
========================================================= */
...
async function handleLoadPreviousOperationStatus() {
  ...
  if (
    ...
    showToast(
      "TO·BO1·BO2 업무일지에서만 전 근무 운전현황을 가져올 수 있습니다."
    );
  ...
  try {
    ...
    if (
      ...
      showToast(
        `${currentRole} 전 근무 운전현황을 찾을 수 없습니다.`
      );
...
const loadPreviousHandoverEntriesButton =
  document.getElementById(
    "loadPreviousHandoverEntriesButton"
  );
...
if (
  ...
  loadPreviousHandoverEntriesButton.addEventListener(
    "click",
    async () => {
