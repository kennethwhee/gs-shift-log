...
test("FBHE pending cycles queue on V4 using the Agent-compatible legacy wire snapshot without guessing startup", async () => {
  const f = createFixture();
  const tag = "104HHL60AP621";
  try {
    f.sqlite.prepare(`UPDATE blower_history_assets
      SET cycle_start_state='pending', cycle_started_at=NULL,
          cycle_start_revision='pending-start-r1', cycle_runtime_revision='pending-runtime-r1'
      WHERE tag_number=?`).run(tag);
    const created = await browserCreate(f.database, createBody(tag));
    assert.equal(created.status, 201, JSON.stringify(created.body));
    const probe = created.body.item.probe;
    assert.equal(Date.parse(probe.startAt), Date.parse(f.replacementAt));
    assert.equal(probe.expectedCycleStartState, 'legacy');
    assert.equal(probe.expectedCycleStartedAt, '');
    assert.equal(probe.expectedCycleStartRevision, 'signal-only-v1:pending-start-r1');
    assert.equal(probe.expectedCycleRuntimeRevision, 'pending-runtime-r1');
    const intent = f.sqlite.prepare(`SELECT * FROM blower_runtime_probe_intents_v4 WHERE request_id=?`).get(created.body.item.id);
    assert.equal(intent.expected_cycle_start_state, 'legacy');
    assert.equal(intent.expected_cycle_start_revision, 'signal-only-v1:pending-start-r1');
    assert.throws(() => f.sqlite.prepare(`SELECT * FROM blower_runtime_probe_intents_v5`).all(), /no such table/i);

    const claimed = await agentClaim(f.database);
    assert.equal(claimed.status, 200);
    assert.equal(claimed.body.items.excel.probe.expectedCycleStartState, 'legacy');
    const completed = await agentComplete(f.database, created.body.item.id, zeroRuntimeResult(claimed.body.items.excel));
