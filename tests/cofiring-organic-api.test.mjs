...
const api=await import('data:text/javascript;base64,'+fs.readFileSync(new URL('../functions/api/cofiring-organic-usage.js',import.meta.url)).toString('base64'));
...
async function call(d,{method='GET',payload=null,headers={},query='2026-09-08',auth=true,raw=null}={}){
 const r=new Request('https://shift.test/api/cofiring-organic-usage?targetDate='+encodeURIComponent(query),{method,headers:{...(auth?{Authorization:'Bearer '+token}:{}),'Content-Type':'application/json','X-ShiftLog-Client':'desktop',...headers},...(['GET','HEAD'].includes(method)?{}:{body:raw??JSON.stringify(payload)})});
