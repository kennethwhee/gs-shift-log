(function(root){
  'use strict';
  const API='/api/cofiring-manure-usage';
  const UNITS=['unit1','unit2'];
  function parseTons(value){
    if(typeof value!=='string' && typeof value!=='number')throw new Error('축분 사용량을 입력해 주세요.');
  ...
  function create(options){
    ...
    async function request(url,init){
      ...
      try{
        ...
        if(!response.ok||payload?.ok!==true){const e=new Error(payload?.message||'축분 저장 요청을 처리하지 못했습니다.');e.status=response.status;e.payload=payload;throw e;}
        return payload;
  ...
  root.CofiringManureStorage={create,parseTons,validDate,validRecord};
  if(typeof module==='object'&&module.exports)module.exports=root.CofiringManureStorage;
