(function(root){
  ...
  function mount(container){
    ...
    const adjustButton=container.querySelector('[data-cfv56-adjust]');if(adjustButton){adjustButton.disabled=true;adjustButton.addEventListener('click',async()=>{try{const morningView=container.closest?.('[data-efficiency-view="morning-meeting"],#efficiencyMorningMeetingView');if(morningView){const reviewApi=root.MorningMeetingCofiringCoalReviewPopupV8;let confirmed=true;if(reviewApi?.confirmBeforeOpen){confirmed=await reviewApi.confirmBeforeOpen();}else if(root.confirm){confirmed=root.confirm('1·2호기 석탄 사용량 검토 필요\n\n혼소 조정 창을 열기 전에 1호기·2호기 Coal 사용량을 확인해 주세요.');}if(!confirmed)return;}await Promise.resolve(adjuster?.open());}catch(e){setStatus(container,e.message,'error');}});}
