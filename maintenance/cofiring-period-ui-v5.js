(function(root){
  ...
  // COFIRING_RESULTS_TARGET_V1: an operator's browser preference, not shared plant settings.
  const BIO_TARGET_KEY='gs-shift-log.cofiring.bio-target-percent.v1';
  const bioTargets=new WeakMap();
  const targetPercentFor=container=>bioTargets.get(container)??25;
  function parseBioTarget(value){
    const text=String(value??'').trim(),n=Number(text);
    return /^(?:\d+(?:\.\d{1,2})?|\.\d{1,2})$/.test(text)&&Number.isFinite(n)&&n>=0.01&&n<=99.99?n:null;
  }
  function storedBioTarget(){
    try{return parseBioTarget(root.localStorage?.getItem(BIO_TARGET_KEY))??25;}catch(_){return 25;}
  }
  function paintBioTarget(container){
    const value=targetPercentFor(container);
    for(const el of container.querySelectorAll('[data-cfv-target-label]'))el.textContent=value+'%';
  }
  function markup(){
  ...
  function summaryPlaceholder(targetPercent=25){
    const cards=UNITS.map((unit,i)=>`<article class="cfv52-card" data-cfv52-unit="${unit}"><header><div class="cfv52-unit-title"><span class="cfv52-unit-number" aria-hidden="true">0${i+1}</span><div><strong>${i+1}호기</strong></div></div><span class="cfv52-card-status is-waiting">조회 전</span></header><div class="cfv52-metrics">
      ...
      ${summaryMetric('유기성 · 축분 혼소율',null,{ratio:true})}
      ${summaryMetric('종합 혼소율',null,{ratio:true,emphasis:true})}
      </div>${fuelUsageMarkup(null)}${targetReferenceMarkup({targetPercent},{unit})}</article>`);
    cards.push(`<article class="cfv52-card cfv52-card-total"><header><strong>1·2호기 종합 혼소율</strong><span>열량 가중 기준</span></header><div class="cfv52-total-ratios">${summaryMetric('바이오 혼소율',null,{ratio:true})}${summaryMetric('유기성 · 축분 혼소율',null,{ratio:true})}${summaryMetric('종합 혼소율',null,{ratio:true,emphasis:true})}</div></article>`);
    return cards.join('');
  }
  function targetReferenceMarkup(reference,{open=false,unit='unit1'}={}){
    const ready=reference?.ready===true,closed=reference?.status==='closed';
    const goal=escapeHtml(reference?.targetPercent??25);
  ...
  function readManual(container){
    ...
    if(container.dataset.cfvUsageInputMode==='manual')out.inputMode='manual';
  ...
  function writeManual(container,values){
    container.dataset.cfvUsageInputMode=values?.inputMode==='manual'?'manual':'auto';
    if(values?.inputMode==='manual')for(const input of container.querySelectorAll('[data-cfv5-receipt]'))delete input.dataset.cfv14Auto;
    for(const unit of UNITS){
      for(const fuel of ['organic','manure']){
  ...
  function renderSummary(container,result,manualValues,targetError=''){
    ...
    const restoreEditor=retainBioTargetEditor(container);
    const time=container.querySelector('[data-cfv-results-time]');
    if(time)time.textContent=result?.period?.endLocal?result.period.endLocal.replaceAll('-','.').replace('T',' · ')+' 기준':'';
    if(!result){host.innerHTML=summaryPlaceholder(targetPercent);restoreEditor();if(note)note.textContent='DataPARC 조회 전';return;}
    const cards=[];
    for(const [i,unit] of UNITS.entries()){
      ...
      cards.push(`<article class="cfv52-card" data-cfv52-unit="${unit}"><header><div class="cfv52-unit-title"><span class="cfv52-unit-number" aria-hidden="true">0${i+1}</span><div><strong>${i+1}호기</strong></div></div><span class="cfv52-card-status${manualComplete&&bioReady?'':' is-waiting'}">${manualComplete&&bioReady?'계산 완료':bioReady?'종합 계산 대기':'조회값 확인'}</span></header><div class="cfv52-metrics">
        ...
        ${summaryMetric('유기성 · 축분 혼소율',u?.fuelRatios?.organicGroup,{ratio:true})}
        ${summaryMetric('종합 혼소율',u?.fuelRatios?.total,{ratio:true,emphasis:true})}
      </div>${fuelUsageMarkup(u)}${targetReferenceMarkup(targetError?{status:'invalid_input',ready:false,message:targetError,targetPercent}:deadlineTargetApi?{...deadlineTargetApi.forUnit(u,result.period,{targetPercent}),ratioPercent:coalBioRatio(u)}:{targetPercent},{unit,open:host.querySelector?.('[data-cfv52-unit="'+unit+'"]')?.querySelector?.('[data-cfv10-target-details]')?.open===true})}</article>`);
    }
    cards.push(`<article class="cfv52-card cfv52-card-total"><header><strong>1·2호기 종합 혼소율</strong><span>열량 가중 기준</span></header><div class="cfv52-total-ratios">
      ...
      ${summaryMetric('유기성 · 축분 혼소율',result?.combined?.fuelRatios?.organicGroup,{ratio:true})}
  ...
  function retainBioTargetEditor(container){
    ...
    if(editor)container.querySelector('[data-cfv-target-home]')?.appendChild?.(editor);
    return ()=>{
      if(!editor||editor.hidden)return;
      const unit=editor.dataset.unit,slot=container.querySelector('[data-cfv-target-slot="'+unit+'"]');
      if(!slot){editor.hidden=true;return;}
      slot.appendChild?.(editor);
      container.querySelector('[data-cfv-target-edit="'+unit+'"]')?.setAttribute?.('aria-expanded','true');
  ...
  function mount(container){
    ...
    const usageFields=()=>[...container.querySelectorAll('[data-cfv5-manual],[data-cfv5-receipt]')];
    function paintUsageEditor(blocked){
      const editing=!!usageEditSnapshot;
      container.dataset.cfvUsageEditing=String(editing);
      const edit=container.querySelector('[data-cfv5-manual-edit]');
      if(edit){edit.disabled=blocked||editing;edit.setAttribute('aria-pressed',String(editing));}
      const revert=container.querySelector('[data-cfv5-manual-revert]');
      if(revert)revert.disabled=blocked||!editing;
      for(const input of usageFields()){
        input.readOnly=!editing;input.setAttribute('aria-readonly',String(!editing));
      }
    }
    ...
    function updateOrganicInventoryUsage(){
      let spec=null;try{spec=currentSpec();}catch(_){}
      const state=manual?.state();
      const direct=container.dataset.cfvUsageInputMode==='manual';
      const blocked=mobile||!state?.canEdit||!state.loaded||state.loading||state.saving;
      paintUsageEditor(blocked);
      if(direct){
        resetReceiptAuto();
        for(const input of usageFields())input.placeholder='0';
        paintUsageEditor(blocked);
        let values=null;try{values=readManual(container);}catch(_){}
        const total=core.organicManualUsage(values,spec);organicUsageState=total;
        receiptSource('직접 입력','manual');
        organicUsageSource(total.ok?'유기성 총 '+num(total.usage,3)+'t · 직접 입력':total.message,total.ok?'manual':'error');
        const save=container.querySelector('[data-cfv5-manual-save]');if(save)save.disabled=blocked||!total.ok;
        return total;
      }
      if(!state?.loaded||state.loading){
        organicUsageState={ok:false,message:'저장된 사용량 확인 중'};
        const save=container.querySelector('[data-cfv5-manual-save]');if(save)save.disabled=true;
        return organicUsageState;
      }
      const receiptInput=container.querySelector('[data-cfv5-receipt="organic"]');
      const rawReceipt=String(receiptInput?.value??'').trim();
      const signature=spec?JSON.stringify({startLocal:spec.startLocal,endLocal:spec.endLocal}):'';
      const daily=queryMode(container)==='daily';
      const receipt=daily?(receiptSyncState.status==='ready'&&receiptSyncState.signature===signature?receiptSyncState.value:null):
        rawReceipt===''?null:Number(rawReceipt);
      const total=core.organicInventoryUsage(reference,receipt,spec);
      organicUsageState=total;
      let autoChanged=false;
      for(const unit of UNITS){
        const input=container.querySelector('[data-cfv5-manual="'+unit+':organic"]');
        if(!input)continue;
      ...
      if(!total.ok){
        organicUsageSource(total.message+(reference?' · 이전 저장 사용량은 계산에서 제외':''),reference?'error':'');
        const save=container.querySelector('[data-cfv5-manual-save]');if(save)save.disabled=true;
        return total;
      }
      if(autoChanged){manualDirty=true;const label=container.querySelector('[data-cfv5-manual-state]');
        if(label&&!manual?.state()?.saving)label.textContent='유기성 50:50 자동배분 · 미저장';}
      const save=container.querySelector('[data-cfv5-manual-save]');
      if(save)save.disabled=blocked;
      const {startTotal,endTotal,usage}=total;
      const {unit1,unit2,ok:allocationOk}=total.allocation;
      organicUsageSource('유기성 총 '+num(usage,3)+'t · 1호기 '+num(unit1,4)+'t / 2호기 '+num(unit2,4)+'t · 50:50 자동배분','ready');
      renderOrganicBoundaryDetail(reference.organicInventory,usage,startTotal,receipt,endTotal,unit1,unit2,allocationOk);
      return total;
    }
    function validateOrganicAllocationBeforeSave(values){
        if(values?.inputMode==='manual'){
          const total=core.organicManualUsage(values,currentSpec());
        ...
        if(Math.abs(diff)>0.01||Math.abs(one-total.allocation.unit1)>0.000001||Math.abs(two-total.allocation.unit2)>0.000001){
          throw new Error(
      ...
      const generation=++receiptSyncGeneration;receiptSyncState={status:'loading',signature:'',value:null};
      if(container.dataset.cfvUsageInputMode==='manual'){resetReceiptAuto();updateOrganicInventoryUsage();return true;}
      ...
      receiptSource('입고기록 확인 중','working');updateOrganicInventoryUsage();if(reference&&storesReady())calculate();
      try{
        ...
        if(disposed||container.dataset.cfvUsageInputMode==='manual'||generation!==receiptSyncGeneration||epoch!==selectionEpoch||queryMode(container)!=='daily'||container.querySelector('[data-cfv7-date]')?.value!==date||current!==specSignature||String(authHeaders().Authorization||authHeaders().authorization||'')!==auth)return false;
        if(payload.receiptStart!==p.startLocal||payload.receiptEnd!==p.endLocal||payload.source!=='solid-fuel-unloading'||payload.basis!=='completed-unloading-departure')throw new Error('입고량 조회 범위가 다릅니다.');
        setReceiptAutoValues(payload.receipts);receiptSyncState={status:'ready',signature:specSignature,value:payload.receipts.organic};
        ...
        if(disposed||container.dataset.cfvUsageInputMode==='manual'||generation!==receiptSyncGeneration||epoch!==selectionEpoch)return false;
        receiptSyncState={status:'error',signature:specSignature,value:null};
        receiptSource('입고량 확인 실패 · 다시 조회해 주세요','error');updateOrganicInventoryUsage();if(reference&&storesReady())calculate();
        return false;
      }
    }
    ...
    function refreshStatusLine(){
      ...
      else if(hasResult&&organicUsageState?.ok!==true){parts.push('유기성 자료 확인 필요');tone='error';}
    ...
    function showDayUnavailable(){
      ...
      for(const el of container.querySelectorAll('[data-cfv5-calorific],[data-cfv5-coefficient],[data-cfv5-manual],[data-cfv5-receipt],[data-cfv5-settings-save],[data-cfv5-manual-save],[data-cfv5-manual-edit],[data-cfv5-manual-revert],[data-cfv56-adjust]'))el.disabled=true;
    ...
    function scheduleDeadlineRefresh(){
      ...
      const targets=UNITS.map(unit=>deadlineTargetApi.forUnit(displayResult.units[unit],displayResult.period,{targetPercent:targetPercentFor(container)})).filter(t=>t.ready),deadline=Math.min(...targets.map(t=>t.deadlineMs));
    ...
    function setBioTargetEditor(unit,open){
      if(disposed||!targetEditor)return;
      const slot=UNITS.includes(unit)?container.querySelector('[data-cfv-target-slot="'+unit+'"]'):null;
      const visible=!!(open&&slot);
      for(const button of container.querySelectorAll('[data-cfv-target-edit]'))button.setAttribute('aria-expanded',String(visible&&button.getAttribute('data-cfv-target-edit')===unit));
      targetEditor.hidden=!visible;
    ...
    container.querySelector('[data-cfv-target-cancel]')?.addEventListener('click',()=>setBioTargetEditor(targetEditor?.dataset.unit,false));
    function applyBioTarget(){
      if(disposed)return;
      const editedUnit=targetEditor?.dataset.unit;
      const feedback=container.querySelector('[data-cfv-target-feedback]'),value=parseBioTarget(targetInput?.value);
      if(value===null){
        targetInput?.setAttribute?.('aria-invalid','true');
        if(feedback){feedback.dataset.tone='error';feedback.textContent='0.01~99.99%를 소수 둘째 자리까지 입력해 주세요. 현재 목표 '+targetPercentFor(container)+'%를 유지합니다.';}
        targetInput?.focus?.();return;
      }
      flushInputRecalc();
      bioTargets.set(container,value);targetInput.value=String(value);targetInput.setAttribute?.('aria-invalid','false');
      let saved=false;try{if(root.localStorage){root.localStorage.setItem(BIO_TARGET_KEY,String(value));saved=true;}}catch(_){}
      paintBioTarget(container);
      renderSummary(container,displayResult||lastResult,currentManualFromFields(),deadlineInputError);
      if(adjustmentActive){const note=container.querySelector('[data-cfv52-summary-note]');if(note)note.textContent='혼소 조정 적용';}
      scheduleDeadlineRefresh();
      if(editedUnit)setBioTargetEditor(editedUnit,false);
      if(feedback){feedback.dataset.tone='';feedback.textContent=value+'% 적용'+(saved?'':' · 이번 화면에서만 유지');}
    }
    container.querySelector('[data-cfv-target-apply]')?.addEventListener('click',applyBioTarget);
    targetInput?.addEventListener('keydown',event=>{if(event.key==='Enter'){event.preventDefault();applyBioTarget();}else if(event.key==='Escape'){event.preventDefault();setBioTargetEditor(targetEditor?.dataset.unit,false);}});
    ...
    async function restoreClosedManualSnapshot(){
      ...
      */
      ...
      if(snapshot.manual?.inputMode==='manual'){restored.inputMode='manual';restored.receipts=snapshot.manual.receipts;}
      writeManual(
        container,
        restored
      );
    ...
    function paintManual(force=false){if(!manual||showDayUnavailable())return;const s=manual.state(),label=container.querySelector('[data-cfv5-manual-state]');if((force||(!manualDirty&&!usageEditSnapshot))&&s.loaded)writeManual(container,s.values);if(label)label.textContent=s.error?s.error:s.saving?'저장 중...':s.loading?'불러오는 중...':usageEditSnapshot?'수정 중 · 저장 전':manualDirty?'수정됨 · 미저장':s.revision?`저장 v${s.revision}${s.updatedByName?' · '+s.updatedByName:''}`:'저장값 없음';container.querySelector('[data-cfv5-manual-save]').disabled=mobile||!s.canEdit||s.saving;for(const el of container.querySelectorAll('[data-cfv5-manual],[data-cfv5-receipt]'))el.disabled=mobile||!s.canEdit||!s.loaded||s.loading||s.saving;updateOrganicInventoryUsage();const values=currentManualFromFields();renderOrganic(container,displayResult||lastResult,values);renderSummary(container,displayResult||lastResult,values,deadlineInputError);bindManualInputs();}
    ...
    function scheduleInputRecalc(){
      clearInputRecalc();
      if(container.dataset.cfvUsageInputMode==='manual')updateOrganicInventoryUsage();
    ...
    function paintLive(s){
      ...
      else if(item?.saved&&reference&&!organicUsageState?.ok)setStatus(container,organicUsageState?.message||'유기성 재고 자료를 확인해 주세요.','error');
      else if(item?.saved&&reference)setStatus(container,qualityGapSaved?'DataPARC 중간 품질 공백이 있어도 시작·종료 누적 경계가 정상인 사용량은 표시합니다. [자료 확인 내용]에서 품질 공백 시간을 확인해 주세요.':lastResult?.warnings?.length?'저장값으로 혼소율을 계산했습니다. 빈칸 축분은 0t로 계산하며 자료 품질 경고는 [자료 확인 내용]에서 확인해 주세요.':'저장된 DataPARC 결과를 불러와 혼소율 계산까지 완료했습니다. 빈칸 축분은 0t로 계산됩니다.','success');
      else if(!item?.saved)setStatus(container,'저장된 결과가 없습니다. [계산하기]를 누르면 현재 표시 기간으로 조회합니다.','');
    }
    function analyze(){if(!reference)return null;const p=periodSpec(container),setting=readSettings(container),mv=manualForCalculation(readManual(container)),calorifics={unit1:{},unit2:{}},coefficients={unit1:{},unit2:{}};for(const u of UNITS){mv[u].organic=organicUsageState?.ok?organicUsageState.allocation[u]:null;for(const fuel of FUEL_KEYS){calorifics[u][fuel]=setting[u][fuel].calorific;coefficients[u][fuel]=setting[u][fuel].coefficient;}}const result=core.analyzePeriodSummary(reference,{startLocal:p.startLocal,endLocal:p.endLocal,calorifics,coefficients,organic:{start:p.start,end:p.end,unit1:mv.unit1.organic,unit2:mv.unit2.organic},manure:{start:p.start,end:p.end,unit1:mv.unit1.manure,unit2:mv.unit2.manure}});result.organicUsage=organicUsageState;if(!organicUsageState?.ok)result.warnings.push(organicUsageState?.message||'유기성 재고 확인 필요');return result;}
    function calculate(){
      try{
        updateOrganicInventoryUsage();lastResult=analyze();if(!lastResult)return null;deadlineInputError='';
    ...
    container.querySelector('[data-cfv5-manual-edit]').addEventListener('click',()=>{
      const state=manual?.state();
    ...
    container.querySelector('[data-cfv5-manual-revert]').addEventListener('click',async()=>{
      ...
      for(const field of snapshot.fields){
        const input=container.querySelector(field.selector);if(!input)continue;
        input.value=field.value;
        if(field.auto===undefined)delete input.dataset.cfv14Auto;else input.dataset.cfv14Auto=field.auto;
      }
      receiptSource(snapshot.receiptText,snapshot.receiptTone);
      // Re-render without replacing the captured values with older saved values.
      const originalDirty=manualDirty;manualDirty=true;paintManual();manualDirty=originalDirty;
      if(reference&&storesReady())calculate();
      const label=container.querySelector('[data-cfv5-manual-state]');
      if(label)label.textContent=manualDirty?'수정 전 값으로 원복 · 미저장':'수정 전 값으로 원복';
      // An automatic receipt request invalidated by Edit needs a fresh read on cancel.
      if(snapshot.mode==='auto'&&receiptSyncState.status!=='ready')await syncReceiptTotals();
    });
    container.querySelector('[data-cfv5-manual-save]').addEventListener('click',async()=>{
      try{
    ...
    container.querySelector('[data-cfv5-query]').addEventListener('click',async()=>{if(clickBusy)return;clickBusy=true;fastPrepGeneration++;restoringSaved=false;if(fastPrepTimer){root.clearTimeout?.(fastPrepTimer);fastPrepTimer=null;}let token=null;try{
      ...
      if(cached?.organicInventory&&!live.state().item?.active&&selectedStoreKey===storeKey()&&storesReady()){
        ...
        if(result&&organicUsageState?.ok)setStatus(container,'저장된 DataPARC 결과와 현재 입력값으로 즉시 재계산했습니다. 최신 데이터 조회는 [재조회]를 사용하세요.','success');
        return;
      }
